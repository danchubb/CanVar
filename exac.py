...
#    SITES_VCFS=glob.glob(os.path.join(os.path.dirname(__file__), EXAC_FILES_DIRECTORY, 'all_sites.vcf.gz')),
    #SITES_VCFS=glob.glob(os.path.join(os.path.dirname(__file__), EXAC_FILES_DIRECTORY, 'NSCCG_TESTES.vcf.gz')),
    ...
    BASE_COVERAGE_FILES=glob.glob(os.path.join(os.path.dirname(__file__), EXAC_FILES_DIRECTORY, 'coverage', 'all_coverage.txt.gz')),
...
def parse_tabix_file_subset(tabix_filenames, subset_i, subset_n, record_parser):
    ...
    for tabix_file, contig in tabix_file_contig_subset:
        ...
        print("Iterating pairs",header_iterator)
...
def load_base_coverage():
    def load_coverage(coverage_files, i, n, db):
        print("covfiles",coverage_files,"i",i,"n",n)
    ...
    #print(app.config['BASE_COVERAGE_FILES'])
...
def load_variants_file():
    def load_variants(sites_file, i, n, db):
        ...
        try:
            #var=grouper(1000,variants_generator)
            #db.variants.insert(var, w=0)
            for var in variants_generator:
                #print var
                db.variants.insert(var, w=0)
...
#        #Modify to not bulk insert which results in too many transactions
...
def load_gene_models():
    ...
    with gzip.open(app.config['GENCODE_GTF']) as gtf_file:
		...
		for transcript in get_transcripts_from_gencode_gtf(gtf_file):
			db.transcripts.insert(transcript,w=0)
			print transcript
        #db.transcripts.insertMany((transcript for transcript in get_transcripts_from_gencode_gtf(gtf_file)), w=0)
...
def inject_populations():
    return dict(populations=['NSCCG'])
@app.context_processor
def inject_num_males():
    return dict(num_males=547)
@app.context_processor
def inject_num_females():
...
def get_gene_page_content(gene_id):
    ...
    try:
        ...
        if t is None:
            ...
            coverage_stats = lookups.get_coverage_for_transcript(db, transcript['xstart'] - 100000, transcript['xstop'] + 100000)
            ...
            cache.set(cache_key, t)
            print("did a gene",variants_in_transcript)
        return t
    except Exception, e:
